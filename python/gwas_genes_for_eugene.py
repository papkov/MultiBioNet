...
def add_alz(graph):
    graph = add_vertices(graph, set_of_ensg("../data/GWAS_genes_2.txt"),
                         "GWAS")
    graph.vs["GWAS"] = [True for i in range(graph.vcount())]
...
def delete_vertices_far_from_gwas(graph, max_dist=2):
    dists = [float("Inf") for i in range(graph.vcount())]
    for v in graph.vs.select(GWAS=True):
    ...
    graph.vs["dist"] = dists
    graph.delete_vertices(graph.vs.select(dist_gt=max_dist))
...
def neighbours_of_gwas(graph, max_dist=4):
    ans = {}
    for v in graph.vs.select(GWAS=True):
        ans[v["name"]] = []
        cur_dists = graph.get_shortest_paths(v)
        ...
        for neighbour in range(graph.vcount()):
            if 0 < cur_dists[neighbour] <= max_dist:
                ans[v["name"]].append((graph.vs[neighbour]["name"], cur_dists[neighbour]))
...
if __name__ == '__main__':
    ...
    g = delete_vertices_far_from_gwas(g, 4)

    g.simplify()
    print("- multiedges and loops")
    igraph.summary(g)

    with open('neighbours_of_gwas.pickle', 'wb') as f:
        pickle.dump(neighbours_of_gwas(g), f)
